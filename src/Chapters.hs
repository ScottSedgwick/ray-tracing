...
import Camera (render, renderAA)
import Colour (colour)
import Hit (HitRecord(..), Hittable(..))
import Vec3 (Point3, (<<+), (<<-), (<<**), (<</), dot, unitVec, vlenSquared)
import Ppm (Colour, Ppm, emptyPpm, packPpm, ppm, ppmf)
...
chapter02_1 :: IO()
chapter02_1 = do
  let p' = ppm (emptyPpm 3 2) [colour (1, 0, 0), colour (0, 1, 0), colour (0, 0, 1), colour (1, 1, 0), colour (1, 1, 1), colour (0, 0, 0)]
  B.writeFile "ppm/chapter02_1.ppm" (packPpm p')

chapter02_2 :: IO()
chapter02_2 = do
  let p' = ppmf (emptyPpm 256 256) (\(x,y,_) -> colour $ (x, y, 64) <</ 256)
...
world6 :: [Sphere]
world6 = [ Sphere { centre = (0, 0, -1), radius = 0.5 }
         , Sphere { centre = (0, -100.5, -1), radius = 100 }
         ]

chapter06_2 :: IO()
chapter06_2 = generateFile "ppm/chapter06_2.ppm" $ render (rayColour6_2 world6)
...
chapter07 = do
  g <- getStdGen
  let p' = renderAA g (rayColour7 world6)
  generateFile "ppm/chapter07.ppm" p' 

rayColour7 :: (Hittable a, Foldable t) => t a -> Ray -> Colour
rayColour7 world ray =  f $ hit ray 0 infinity world
  where
    f Nothing = bgcolor ray
    f (Just hitrec) = colour $ normal hitrec <<+ (1,1,1) <<** 0.5